    ...
    def _init_default_distributions(self):
        ...
        dists = {
            ...
            "weight_posterior": DiagonalNormal(
                mean=torch.Tensor(self.out_features, self.in_features).normal_(0, 0.1),
                rho=torch.Tensor(self.out_features, self.in_features).normal_(-5, 0.1),
            ),
        }
        if self.bias:
            dists["bias_prior"] = GaussianMixture(sigma1=0.1, sigma2=0.0005, pi=0.75)
            dists["bias_posterior"] = DiagonalNormal(
                mean=torch.Tensor(self.out_features).normal_(0, 0.1),
