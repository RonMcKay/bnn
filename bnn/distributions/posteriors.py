...
class DiagonalNormal(VariationalDistribution):
    ...
    def log_prob(self, sample, **kwargs):
        ...
        std_dev = self.get_std()
        return (
            torch.log(torch.tensor(2.0 * math.pi)).div(-2.0)
            - std_dev.log()
            - sample.sub(mean).div(std_dev).pow(2).div(2.0)
        )

    def get_std(self):
        return (
            F.softplus(self.last_rho if self.last_rho is not None else self.rho) + 1e-6
        )
...
class Uniform(VariationalDistribution):
    def __init__(
        self, lower_bound=torch.tensor(0.0), upper_bound=torch.tensor(1.0), static=False
    ):
        super().__init__()
        self.log = logging.getLogger(
            __name__[: __name__.rfind(".")] + "." + type(self).__name__
    ...
    def sample(self, **kwargs):
        ...
        return (
            torch.rand_like(lower_bound).mul(upper_bound - lower_bound).add(lower_bound)
        )
    ...
    def log_prob(self, sample, **kwargs):
        ...
        if mask.sum() > 0:
            self.log.error(
                "Encountered values outside of distribution. "
...
class Beta(VariationalDistribution):
    # Not finished yet
    def __init__(
        self, concentration1=torch.tensor(0.5), concentration2=torch.tensor(0.5)
    ):
        super().__init__()
        self.log = logging.getLogger(
            __name__[: __name__.rfind(".")] + "." + type(self).__name__
